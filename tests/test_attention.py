...
from quest.flash_attn_triton import flash_attn_func
from quest.quest_attn import HyperAttention, ReformerAttention
...
def get_arguments():
    ...
    parser.add_argument("--attn_method", type=str, default="flash", choices=['flash', 'flash-cuda', 'hyper', 'hyper-cuda', 'reformer'])
...
def run_reformer_attn(batch_size, seq_len, dim, causal, mode, impl="triton", warmup=20, rep=100):
    q = torch.randn((batch_size, seq_len, dim), dtype=torch.bfloat16, device="cuda", requires_grad=True)
    k = torch.randn((batch_size, seq_len, dim), dtype=torch.bfloat16, device="cuda", requires_grad=True)
    v = torch.randn((batch_size, seq_len, dim), dtype=torch.bfloat16, device="cuda", requires_grad=True)

    attn_reformer = ReformerAttention(
        dropout = 0.,
        bucket_size =128,
        n_hashes = 16,
        causal = causal).to(device='cuda', dtype=q.dtype)
    
    fn_ref = lambda: attn_reformer(q, v, query_len=q.shape[1])

    if mode == 'fwd':
        return triton.testing.do_bench(fn_ref, warmup=warmup, rep=rep, quantiles=[0.2, 0.5, 0.8])
...
def main():
    ...
    for seq_len in seq_lens:
        ...
        elif attn_method == "reformer":
            ms = run_reformer_attn(batch_size, seq_len, dim, causal, mode=args.mode)
        else:
