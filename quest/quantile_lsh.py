...
class LSH(torch.nn.Module):
    def __init__(self, 
        ...
        self.causal = causal
        self.q_digest = QuantileDigest(0.8)

    def sort_key_val(self, t1, t2, dim=-1):
        values, indices = t1.sort(dim=dim) #add to qd
        t2 = t2.expand_as(t1) #reshape
        return values, t2.gather(dim, indices) #query qd to get index, then query t2
    ...
    def sort_qdigest(self, vecs, dim=-1):
        self.add_values(self.q_digest, vecs[dim])
    ...
    def hash_vectors(self, vecs, v, input_mask=None, input_attn_mask = None, pos_emb = None):
        ...
        print(sbuckets_and_t, sticker.shape, undo_sort.shape)
        del ticker
